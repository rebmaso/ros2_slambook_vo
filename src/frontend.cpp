...
bool Frontend::AddFrame(Frame::Ptr & frame) {
    ...
    switch (status_) {
        case FrontendStatus::INITING:
            StereoInit();
            break;
        // Track() called if tracking either rgood or bad
        case FrontendStatus::TRACKING_GOOD:
        case FrontendStatus::TRACKING_BAD:
            Track();
            break;
        case FrontendStatus::LOST:
            Reset(); // just try again
            break;
    }
...
bool Frontend::InsertKeyframe() {
    ...
    // if few inlierrs --> current frame is a new keyframe
    ...
    // for the few tracked features, mark landmarks as observed in current frame
    ...
    FindFeaturesInRight();
    // from left-rright corresps, triangulate map points
    TriangulateNewPoints();
    // update backend because we have a new keyframe
    backend_->UpdateMap();
    loopclos_->NewKF(current_frame_);
...
// mark each detected landmark as observed
// this is for the few featurres that arre tracked
...
// estimate current pose via pnp with g2o
// it is set as motion-only BA:
// there are only pose vertices. landmarks are fixed.
int Frontend::EstimateCurrentPose() {
    ...
    std::vector<Feature::Ptr> features; // points to features in curr image
    for (size_t i = 0; i < current_frame_->features_left_.size(); ++i) {
    ...
    const double chi2_th = 5.991; // rreprroj threshold
    int cnt_outlier = 0;
    for (int iteration = 0; iteration < 4; ++iteration) {
        ...
        // count the outliers
        // outliers if outside error threshold. but no ransac done here
        // just discarrding them once
        for (size_t i = 0; i < edges.size(); ++i) {
            ...
            // No need for robust kernel after iter 2 (expect lower errors)
            if (iteration == 2) {
    ...
    return features.size() - cnt_outlier; // return number inliers
}
...
// Detect new features at new keyframe
